...
posts = {
    ...
    json: function json(options) {
         var attrs = ['id', 'slug', 'status','published_at'],
        ...
        return dataProvider.Post.findAll(data, options).then(function (result) {
            if (result) {
                //return (result.toJSON())
                result = result.toJSON();
                result=(_.map(result, function(x){if(x.status==='published' && !x.page) return {title:x.title, slug:x.slug, image:x.image,published_at:x.published_at}}))
                var cleanFoo = [];
                for (var i in result) {
                  if (result[i] !== undefined) {
                    cleanFoo.push(result[i]);
                  }
                }

                //console.log(cleanFoo)
                return cleanFoo.sort(new2old_sort);
            }
